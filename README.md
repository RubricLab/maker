...
Email links create accounts after verification. Users can then register passkeys at `/auth/passkeys`, also linked from the board's Account link. Email remains available for recovery. No password or email allowlist.
...
The Bun auth proxy protects Next.js, including assets and API routes. Bind both servers to loopback and route public traffic through the auth proxy. Caddy must overwrite `X-Real-IP`. The board stays shared across users.
...
`bun x tsc --noEmit` and `bun --bun run build` check the app.
