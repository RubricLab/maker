...
try {
	...
	assert(firstIcon?.startsWith('data:image/svg+xml,'))
	assert(!decodeURIComponent(firstIcon).includes('pattern'))
	await page.locator('.mobile-game-actions button').last().click()
	await page.getByRole('dialog', { name: 'Game of Life' }).waitFor()
	const life = page.locator('.game-life')
	assert.equal(await life.getAttribute('width'), '25')
	assert.equal(await life.getAttribute('height'), '25')
	assert.equal(await page.locator('.game-cell').count(), 0)
	assert.equal(await life.evaluate(element => getComputedStyle(element).animationDuration), '2s')
	...
	await page.waitForFunction(
		previous =>
			document.querySelector('link[data-maker-canvas-icon]')?.getAttribute('href') !== previous,
		firstIcon
	)
	const clearedIcon = await icon.getAttribute('href')
	assert(!decodeURIComponent(clearedIcon ?? '').includes('<path'))
	for (let i = 0; i < 7; i++) await page.locator('.size-increase').click()
	assert.equal(await page.locator('.pixel-cell').count(), 900)
	await page.getByRole('button', { name: 'Play snake' }).waitFor()
	await page.locator('.mobile-game-actions button').first().click()
	await page.getByRole('dialog', { name: 'Snake' }).waitFor()
	assert.equal(await page.locator('.snake-inline .game-cell').count(), 900)
	assert.equal(await page.locator('.game-overlay').count(), 0)
	assert.equal(await page.locator('.pixel-grid').count(), 0)
	assert.equal(await page.getByRole('slider', { name: 'Game speed' }).count(), 0)
	await page.getByText('Game over').waitFor()
	await page.getByText(/Best \d+/).waitFor()
	await page.getByRole('button', { name: 'Close game' }).click()
