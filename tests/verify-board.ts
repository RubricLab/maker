...
try {
	...
	const credit = await page.locator('.canvas-creator').boundingBox()
	const addButton = await page.locator('.desktop-board-action').boundingBox()
	assert(credit && addButton)
	assert(Math.abs(credit.y + credit.height / 2 - (addButton.y + addButton.height / 2)) < 2)
